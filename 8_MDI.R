...
getParameters <-function(model){ 
  ...
  tryCatch(
    {return(c(summary(model)$coefficients[indexCoefName,],confint.default(model)[indexCoefName,]))}, ## add CI to parameter outputs
...
getParameters2 <-function(model){ 
  tryCatch(
    {return(c(summary(model)$coefficients[indexCoefName2,]))},#,exp(cbind(OR=coef(d),confint.default(d)))[var1_cat3,]))}
    error = function(err){print(err)
...
getParameters3 <-function(model){ 
  tryCatch(
    {return(c(summary(model)$coefficients[indexCoefName3,]))},#,exp(cbind(OR=coef(d),confint.default(d)))[var1_cat3,]))}
    error = function(err){print(err)
      return(c(NA,NA,NA,NA))
    })
}
...
para_to_dtable <- function(list_param, new_var){
  ...
  eval(parse(text=paste("dt_param$OR_", new_var, "<- exp(dt_param$Estimate)", sep="")))
  ...
  if ((sapply(dt_param, class)["grid_id"]) =="integer") {dt_param[, grid_id:= as.character(grid_id)]}   
  return(dt_param)
}
...
dmjoindata = function (distmatrix, id_name, datafiles, uid_name, flagexport=FALSE, outdir="", tablename="") {
  
  ## check data type of parameters
  if (!is(distmatrix, "data.table") & !is(distmatrix, "character")) {
    stop("The parameter for the distance matrix input is not correctly specified. Please refer to a data table object in memory or a valid text file!")
  }
  if (!is(id_name, "character")) { stop("The parameter for the identifier of distance matrix is not a valid string. Please provide a string representing a valid field!")}
  if (!is(datafiles, "character")) { stop("The parameter for the data input is not correctly specified. Please proide a string indicating the path to a valid shapefile or text file!") }
  if (!is(uid_name, "character")) { stop("The parameter for the identifier of data table is not a valid string. Please provide a string representing a valid field!")}  
  if (!is(flagexport, "logical")) { stop("The parameter for writing distance matrix to a table is not a valid logical value. Please provide a TRUE or FALSE value!")}
  if (!is(outdir, "character")) { stop("The parameter for the output path is not correctly specified. Please provide a string indicating a valid path!")}
  if (!is(tablename, "character")) { stop("The parameter for the output table name is not correctly specified. Please provide a string indicating a valid table name!")}        
  ...
  if (is(distmatrix, "data.table")) {
    ...
    if (!file.exists(distmatrix)) {
      stop("The file for distance matrix does not exist. Please double check the input directory (the first parameter) for the data matrix!")
    } else {
      ## if the extension of the input data file is a text file, read the table and check the validity of the input field names   
      if ((file_ext(distmatrix)=="csv") | (file_ext(distmatrix)=="txt")) {    
        dfdist_table <- data.table(read.table(distmatrix, header=TRUE, sep=',', stringsAsFactors=FALSE,comment.char=""))         
      }
    }
  }  

  if(!id_name %in% names(dfdist_table)){
    stop("The field '",id_name,"' does not exist in the data matrix table. Please provide only valid column names.")
  } else {
  ...
  if (!file.exists(datafiles)) {
    stop("The data file does not exist. Please double check the input directory (the third parameter) for the data!")
  } else {
    ## if the extension of the input data file is a text file, read the table and check the validity of the input field names   
    if ((file_ext(datafiles)=="csv") | (file_ext(datafiles)=="txt")) {    
      datatable <- data.table(read.table(datafiles, header=TRUE, sep=',', stringsAsFactors=FALSE,comment.char=""))
  ...
  if(!uid_name %in% names(datatable)){
    stop("The field '",uid_name,"' does not exist in the data file. Please provide only valid column names.")
  } else {
    id_val1 <-  eval(parse(text=paste("datatable$",uid_name)))
    if (any(duplicated(id_val1))) {
      stop("The column '",pntid_name,"' have duplicate values in the spatial point data. Please provide only an valid unique column name.")
    } else {
      ## set the key for the data table
  ...
  if (flagexport) {    
    if (!file.exists(outdir)) {
      stop("The directory for the output file does not exist. Please double check the output directory!")
    } else {
      #if (file.exists(paste(outdir,"/", tablename, ".csv", sep=""))) {warning("The output file exists. Please specify a different file name!")}
      if (file.exists(paste(outdir,"/", tablename, ".csv", sep=""))) {warning("The output file exists. Be aware it will be overwritten!")}
    }         
  }    
  ...
  if ((sapply(datatable, class)["GeoID"]) =="integer") {datatable[, GeoID:= as.character(GeoID)]}   ## the varialbe name is pre-defined, not a variable 
  
  setnames(dfdist_table, id_name, "GeoID")
  if ((sapply(dfdist_table, class)["GeoID"]) =="integer") {dfdist_table[, GeoID:= as.character(GeoID)]}
  ### merge the data matrix table with the data file
  dfdist_merge_table <- merge(dfdist_table, datatable, by = "GeoID", all.x=TRUE) 

  ## export the distance matrix tabel as an csv file, if the user wants to do so
  if (flagexport) {
    write.csv(dfdist_merge_table, file = paste(outdir,"/", tablename, ".csv", sep=""), row.names=FALSE)
  }
...
outMatrix <- data.frame(MSA=character(), NumBGs=integer(), Num_largeSE5 = integer(), Perc_racesig = double(), Perc_lirsig = double(), Perc_sexsig = double(),
                        Filter_Max = double(), Filter_Mean  = double(), Filter_Median = double(), stringsAsFactors=FALSE)   ## the last parameter is used for adding character columns without the warning (invalid factor level, NA generated)  
...
dfdist_table <- data.table(col = rep(colnames(mdist), each = nrow(mdist)), 
...
if ((sapply(sp_grd_point_table, class)["grid_id"]) =="integer") {sp_grd_point_table[, grid_id:= as.character(grid_id)]}
dfdist_merge_table <- merge(dfdist_table, sp_grd_point_table, by = "grid_id", all.x=TRUE)
msaDistMatrix <- dfdist_merge_table
print(paste("Completed",Sys.time()))

print("Joining Distance Matrix")
msaDistmatrix_join <- dmjoindata(msaDistMatrix, "GeoID", paste(outputDSN, "/", outputFileCases.centroid, ".shp",sep = ""),"GEOID")
print(paste("Completed",Sys.time()))
###########################################################################################################  
...
hmda_table <- data.table(read.table(paste(outputDSN,"/",hmdaFile,sep = ""),
...
levels(hmda_table$race) = gsub("American Indian or Alaska Native", "Indian", levels(hmda_table$race))
levels(hmda_table$race) = gsub("Black or African American", "Black", levels(hmda_table$race))
levels(hmda_table$race) = gsub("Native Hawaiian or Other Pacific Islander", "Pacific Islander", levels(hmda_table$race))
...
hmda_table$combRace <- factor(hmda_table$combRace, levels = c("White", "Black", "Hispanic", "Asian",  "Indian", "Pacific Islander"))
...
if(indexQuery == REDLINING_WITHOUT_RACE){
  ...
  colnames(hmda_table.apps) <- c("GeoID","numWhite", "numBlack", "numHisp", "numAsian", "numIndian", "numPI", "numNA")
  ...
  colnames(hmda_table.denies) <- c("GeoID","denyWhite", "denyBlack", "denyHisp", "denyAsian", "denyIndian", "denyPI", "denyNA")    #colnames(hmda_table.denies) <- c("GeoID","denyWhite", "denyBlack", "denyHisp", "denyAsian", "denyIndian", "denyPI")
  hmda_table.sum<-merge(hmda_table.apps,hmda_table.denies, by = "GeoID", all.x=TRUE)
  ...
  colnames(hmda_table.approves) <- c("GeoID","apprWhite", "apprBlack", "apprHisp", "apprAsian", "apprIndian", "apprPI","apprNA")    #colnames(hmda_table.approves) <- c("GeoID","apprWhite", "apprBlack", "apprHisp", "apprAsian", "apprIndian", "apprPI")
  hmda_table.sum<-merge(hmda_table.sum,hmda_table.approves, by = "GeoID", all.x=TRUE)
...
dfdist_table <- merge(msaDistmatrix_join, hmda_table.sum, by = "GeoID", all.x=TRUE)
dfdist_table[, eval(parse(text=paste(gridid_name, ":=as.character(", gridid_name, ")", sep="")))]
print(paste("Completed",Sys.time()))

# Sort the data table by the grid id column, then distance 
print("Creating Grid and Setting Filters")
grid_point_case <- dfdist_table[eval(parse(text=paste("order(", gridid_name, ",", dist_name, ")" ,sep=""))),]
...
grid_point_case[,eval(parse(text=paste("cum_cases1:=cumsum(",keyvar1, ")",sep=""))), by = eval(parse(text=paste("list(", gridid_name, ")", sep="")))]  
grid_point_case[,eval(parse(text=paste("cum_cases2:=cumsum(",keyvar2, ")",sep=""))), by = eval(parse(text=paste("list(", gridid_name, ")", sep="")))]

## Flag the first row for each grid id group
grid_point_case[,eval(parse(text=paste("first := !duplicated(grid_point_case[,", gridid_name, "])", sep="")))]  
...
grid_point_case[((threshold==0) & (cum_cases_p1< dist_threshold1) & !(first)), threshold:=2]
grid_point_case[((threshold==0) & (cum_cases_p2< dist_threshold2) & (cum_cases_p1 >= dist_threshold1) & !(first)), threshold:=3]
...
grid_point_case1_plus <- merge(grid_point_case1, hmda_table, by = "GeoID", allow.cartesian=TRUE)

grid_point_case1_plus$member <- factor(grid_point_case1_plus$member, levels=c(0,1), labels=c("outside", "inside"))
print(paste("Completed",Sys.time()))
############################################################################################################
...
if (indexQuery == REDLINING_WITHOUT_RACE) {
  sampleTime<-system.time(
    modelvector<-lapply(
      split(grid_point_case1_plus,grid_point_case1_plus$grid_id),function(d){ 
        logitmodel <- try(glm(factor(denied) ~ member + loan_income_ratio + factor(app_sex), data= d, family ="binomial")) 
        if (isTRUE(class(logitmodel)=="try-error")){return(NULL) }else {return(logitmodel)} #Check for model convergence 
      }
    )
  )
}
...
if(indexQuery == REDLINING_WITHOUT_RACE){indexCoefName <- "memberinside"}

### extract the attributes from grid data and merge with the model results
if(indexQuery == REDLINING_WITHOUT_RACE) {newvar1 <-"Inside"}
...
aggdata <- grid_point_case[(threshold > 0), eval(parse(text=paste("list(nearDmax = max(near_dist), X = mean(", X_name, "), Y = mean(", Y_name, "), numDenied = sum(", "denyTotal","),  numTotal = sum(","numTotal", "), numBlack = sum(", "numBlack","), numWhite = sum(","numWhite", "), denyBlack = sum(", "denyBlack","), denyWhite = sum(","denyWhite", "), apprBlack = sum(", "apprBlack","), apprWhite = sum(","apprWhite", "), numObs = sum(threshold))", sep=""))), by = grid_id]
...
###############################################  Output Results  ##############################################
if(indexQuery == REDLINING_WITHOUT_RACE){indexname <- paste(dist_threshold1,"da", sep='')}

outputDSNsub <- paste0(outputDSN, "/", y1, "_", y2) 
if (dir.exists(outputDSNsub)==FALSE) dir.create(outputDSNsub)
if (censusyear == "2010") {
  shpname <- paste("Grid2010_",msaName,"nb1_",indexname, "_red_convent",sep="") 
}
st_write(sp_grid_point_chr, paste0(outputDSNsub, "/", shpname, ".shp"), delete_dsn = TRUE)
write.dta(aggdata_plus, file = paste(outputDSNsub, "/redlining_", msaName, "_T", dist_threshold1, "_convent.dta", sep=""))
